...
$sql = "SELECT * FROM course_outcomes WHERE semester = ? AND department = ? AND subject_name = ? AND subject_code = ? AND test_type = ?";
$stmt = $mysqli->prepare($sql);
...
// Part B: questions 11A, 11B, 12A, 12B, 13A, 13B
$partBQuestions = [
    '11a', '11b',
    '12a', '12b',
    '13a', '13b'
...
foreach ($partBQuestions as $question) {
    ...
    // Output to check if values are retrieved correctly
}
